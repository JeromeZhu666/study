 ...
 * 自定义线段树
 * <p>
 * 线段树的定义:用以存储区间或线段,并且允许快速查询结构内包含某一点的所有区间.<br>
 * 1.其每一个叶节点,从左到右代表每个单位区间.<br>
 * 2.其内部节点代表的区间是其两个儿子代表的区间之联集.<br>
 * 3.每个节点（包含叶子）中有一个存储线段的数据结构.若一个线段S的坐标区间包含Int(N)但不包含Int(parent(N)),则节点N中会存储线段S.
 *
 * @author Jerome Zhu
