...
/**
 * 二分搜索树的实现(不包含重复元素)
 * <p>二分搜索树是二叉树,每个叶子节点的值满足:
 * <br>1、大于其左子树所有节点的值.
 * <br>2、小于其右子树所有节点的值.
 * <br>每一颗子树也是二分搜索树,存储的元素必须具有可比较性.
 *
 * <p><br>二叉树的定义:
 * <br>二叉树是一个动态的数据结构,具有唯一的根节点.
 * <br>二叉树的每一个节点最多有两个孩子.
 * <br>二叉树的每一个节点最多有一个父亲.
 * <br>二叉树具有天然的递归性:每个节点左子树也是二叉树,每个节点的右子树也是二叉树.
 * <br>二叉树不一定是"满"的:一个节点也是二叉树,null也是二叉树.
 *
 * @author Jerome Zhu
...
//        else
//            add(root, e);
        root = recursiveAdd(root, e);// 向根节点插入元素,并将新的树赋给根节点
    }
...
//        }
        ...
        if (node == null) {// 最基本的问题,应该插入的位置节点为null
            size++;
            return new Node(e);
        }

        if (e.compareTo(node.e) < 0) // 插入的元素比当前节点小
            node.left = recursiveAdd(node.left, e);// 向当前节点的左子树插入元素,并将新的左子树赋给当前节点
        else if (e.compareTo(node.e) > 0) // 插入的元素比当前节点大
            node.right = recursiveAdd(node.right, e); //向当前节点的右子树插入元素,并将新的右子树赋给当前节点
    ...
    private Node recursiveSearch(Node node, E e) {
        if (node == null)// 已经递归到叶子节点
            return null;

        if (e.equals(node.e)) {
            return node;
        } else if (e.compareTo(node.e) < 0) {
            return recursiveSearch(node.left, e); // 去左子树中匹配
        } else
            return recursiveSearch(node.right, e); // 去右子树中匹配
    }
    ...
    private void stackPreOrder(Node node) {
        ...
        stack.push(node); // 首先将二分查找树的根节点压入栈
        while (!stack.empty()) { // 开始遍历,直到栈中的元素为空
            Node curNode = stack.pop(); // 栈顶元素出栈,访问该元素
    ...
    public void levelOrder() {
        ...
        while (!queue.isEmpty()) { // 当队列不为空
            Node curNode = queue.remove(); // 元素出队
            System.out.println(curNode.e);
...
//                return node.right;
            // 优化
            Node rightNode = node.right;// 拿到待返回的当前节点的右子树
            node.right = null;// 将当前节点从当前树中断开
            size--;// 维护size
